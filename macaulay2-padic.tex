...
FLINT uses two main types for its $p$-adic number module.  A \texttt{padic\_t} object is an array containing a single struct containing $u$, $\nu$, and $N$, and a \texttt{padic\_ctx\_t} is an array containing a struct with information about $p$.  This information is separated so that the same \texttt{padic\_ctx\_t} object may be used for computations involving multiple \texttt{padic\_t} objects.

\begin{tabular}{cc}
  operation & FLINT function \\ \hline
  $-x$ & \texttt{padic\_neg} \\
